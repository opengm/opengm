...
namespace detail_gcg{
   ...
   typename GM::IndexType getCCFromLabels(
      ...
      boost::unordered_map<IndexType,IndexType> repLabeling;
   ...
   void findFirst(
      ...
      typedef boost::unordered_map<ToFindType,size_t> MapType;
   ...
   //dirtyFactors_(gm_.numberOfFactors(),1))
{
   ...
   typedef  boost::unordered_map<UInt64Type,ValueType>  MapType;
   MapType factorMap;
...
CGC<GM, ACC>::greedy2ColoringPlanar(VISITOR & visitor){
   ...
   while(changes){
      ...
      typedef boost::unordered_map< opengm::UInt64Type , IndexType > MapType;
      typedef typename MapType::const_iterator MapIter;
      MapType factorCCs;
      for(IndexType fi=0;fi<numDualVar_;++fi){
