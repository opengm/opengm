...
#include <opengm/inference/cgc/isinf_wrapper.hxx>
...
public:    
...
#if 0
    //remember: if a region has a border that is forced off,
    //          make sure that it gets the same label number as the
    //          adjacent region

    /**
     * localVis:  variable indices for which to construct the submodel 
     * globalArg: helps us to find the border factors, and to know
     *            whether the border should be forced off or kept variable
     */
    template<class ARG>
    IVPairType inferMany(
        ARG & globalArg,
        std::vector<IndexType> localVis,
        IndexType offset
    );
    ...
    growSubmodelCGC(
        size_t maxSize,
        ARG& globalArg,
        IndexType startFactorIdx,
        std::vector<IndexType>& localVis
    ) {
        label1 = getColorOfVI1(startFactorIdx)
        label2 = getColorOfVI2(startFactorIdx)
        
        std::vector<IndexType> r1 = getRegion(label1)
        std::vector<IndexType> r2 = getRegion(label2)
    }
...
SubmodelCGC<GM>::infer2Subsets(
    ...
    if(numSubVar_<= maxBruteForceSize2_ && planar==true){
        ...
        OPENGM_CHECK(planar,"");
        //std::cout<<"do isinf on "<<numSubVar_<<" vars "<<numSubFactors_<<"facs \n";
        //std::cout<<"get embedding graph \n";
        this->getEmbeddingGraph();
        //std::cout<<"infer is inf\n";
...
template<class GM>
void SubmodelCGC<GM>::getEmbeddingGraph(){
    ////////////////////////////////
    // get embedding
    ////////////////////////////////
    OPENGM_CHECK_OP(numSubFactors_, >= , 2 ,"less than 2 valid edges found");
       OPENGM_CHECK_OP(numSubFactors_, <= , 3*numSubVar_ - 6 ,"edge limit for planar graphs is violated");

    //int max_degree=1;

    // create graph in planarity code
    // and fill local edge map
    graph_  = gp_New();
       gp_InitGraph(graph_, numSubVar_);
    for (IndexType fiLocal=0;fiLocal<numSubFactors_;++fiLocal){
        ...
        if (!localEdgemap_.insert(EdgeMapEntry(Edge(localVi0,localVi1), fiLocal)).second) {
              std::cout<<"local v1 "<<localVi0<<"\n";
            std::cout<<"local v2 "<<localVi1<<"\n";
            OPENGM_CHECK(false,"duplicate edge between node");
        }
        int res=gp_AddEdge(graph_,localVi0, 0, localVi1, 0);
        OPENGM_CHECK_OP(res,==,0,"error in gpAddEdge ");
    }

    // timed call to planarity code
    const int n = gp_Embed(graph_, EMBEDFLAGS_PLANAR);
    OPENGM_CHECK_OP(n, ==, 0," error in gp_EmbedR Result");
    gp_SortVertices(graph_);


    // get max-degree from graph 
    //for (IndexType i=0; i<numSubVar_; i++) {
    //    const IndexType j = gp_GetVertexDegree(graph_,i);
    //    if (max_degree < j)
    //        max_degree = j;
    //}
}
...
template<class GM>
typename SubmodelCGC<GM>::ValueType SubmodelCGC<GM>::inferIsInf(){
    typedef std::pair<size_t,size_t> Edge;
    const int n_nodes    = graph_->N;
    OPENGM_CHECK_OP(n_nodes,==,numSubVar_, " internal error");
    OPENGM_CHECK_OP(n_nodes,>,0," ");
    ///////////////////////////////////////////
    // get blossom graph from embedding graph
    ///////////////////////////////////////////
    Isinf::MyBlossomGraph blossomGraph;
    blossomGraph.resizeNodes(numSubVar_);
    blossomGraph.resizeEdges(numSubFactors_);

    OPENGM_CHECK_OP((graph_->N), ==, numSubVar_, " ");
    
    for (int i = 0; i < graph_->N; ++i){
        int j = graph_->G[i].link[1];
        while (j >= graph_->N) {
            Edge e(std::min(i, graph_->G[j].v), std::max(i, graph_->G[j].v));
            EdgeMap::iterator it(localEdgemap_.find(e));
            OPENGM_CHECK(it != localEdgemap_.end(),"cant find an edge");
           OPENGM_CHECK_OP(it->second,<,numSubFactors_, " ");
            blossomGraph.addEdgeAndNode(i,it->second);
            j = graph_->G[j].link[1];
        }
    }
    
    OPENGM_CHECK_OP(localLambdas_.size() ,>= ,blossomGraph.nedges," ");

    /////////////////////////////////////////
    // some (primitve ) assertions
    /////////////////////////////////////////
    const int n_edges = blossomGraph.nedges;
    OPENGM_CHECK_OP(n_edges,==,numSubFactors_, " internal error");
    OPENGM_CHECK(blossomGraph.is_connected()," ");
    int genus=blossomGraph.genus();
    if (genus > 0){
        throw std::runtime_error((boost::format(" \n\n\n !!!!! \n\n warning: graph has genus %d (Not planar)\n\n\n!!!!\n\n") % genus).str());
    }

    OPENGM_CHECK_OP(localLambdas_.size() ,>= ,blossomGraph.nedges," ");
   
    //FIXME FIXME FIXME FIXME
    size_t n = blossomGraph.angulate();
    if (n+numSubFactors_ > localLambdas_.size())
        localLambdas_.resize(n+numSubFactors_);
    
    OPENGM_CHECK_OP(localLambdas_.size() ,>= ,blossomGraph.nedges," ");

    //Isinf::CPUtimer *t = new Isinf::CPUtimer;
    //std::cout << boost::format("    call_blossom with %d sub factors") % numSubFactors_ << std::endl;
    blossomGraph.call_blossom(&localLambdas_[0],numSubFactors_);
    //std::cout << "    ... done calling blossom" << std::endl; 

    //std::vector<double> cost;
    //cost.reserve(blossomGraph.nedges);
    
    OPENGM_CHECK_OP(localLambdas_.size() ,>= ,blossomGraph.nedges," ");
    for (size_t i = 0; i < blossomGraph.nedges; ++i){
        if(localLambdas_[i]>0==false){
            localLambdas_[i]=-1.0*localLambdas_[i];
        }
    }
    blossomGraph.my_mmc_nodestate(localLambdas_,numSubFactors_);
    
    // calculate & compare energies
    double lb = 0.0;
    double ub = 0.0;
    for (size_t i = 0; i < numSubFactors_; ++i){
        if (!blossomGraph.edges[i]->agree){ 
            lb += globalLambdas_[localFactorVis_(i,2)];
        }
        if (blossomGraph.nodes[blossomGraph.edges[i]->node].state != blossomGraph.nodes[blossomGraph.edges[i]->other->node].state){
            ub += globalLambdas_[localFactorVis_(i,2)];
        }
    }
    bool same = (std::abs(lb-ub) <= std::abs(lb)*1e-12);
    
    if (!blossomGraph.genus()) {
        OPENGM_CHECK(same,"");
    }
    else if(!same){
        OPENGM_CHECK_OP(lb ,<= ,ub," ");
    }

    if (!same){
        printf("warning, bounds different (nonplanar?).\n");
    }
    
    // get result
    for (IndexType viLocal = 0; viLocal < blossomGraph.nodes.size(); ++viLocal){
        localArg_[viLocal] =
         (blossomGraph.nodes[viLocal].state != blossomGraph.nodes.back().state ? 1 : 0);
    }
...
template<class GM>
void SubmodelCGC<GM>::freeEmbeddingGraph(){
    localEdgemap_.clear();
    gp_Free(&graph_);
}
