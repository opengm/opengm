#pragma once
#ifndef OPENGM_PYTHON_FUNCTION_HXX
#define OPENGM_PYTHON_FUNCTION_HXX
...
namespace opengm{
   template<class T,class I,class L>
   class FunctionSerialization<PythonFunction<T, I, L> >{
   public:
      typedef typename PythonFunction<T, I, L>::ValueType ValueType;

      static size_t indexSequenceSize(const PythonFunction<T, I, L>&){
      ...
      static size_t valueSequenceSize(const PythonFunction<T, I, L>&){
         throw RuntimeError("Cannot save/load gm with a pure python function: Pure Python Function cannot be serialized / deserialized");
         return 0;
      }
      template<class INDEX_OUTPUT_ITERATOR, class VALUE_OUTPUT_ITERATOR>
      static void serialize(const PythonFunction<T, I, L>&, INDEX_OUTPUT_ITERATOR, VALUE_OUTPUT_ITERATOR){
         throw RuntimeError("Cannot save/load gm with a pure python function: Pure Python Function cannot be serialized / deserialized");
      }
      template<class INDEX_INPUT_ITERATOR, class VALUE_INPUT_ITERATOR>
      static void deserialize( INDEX_INPUT_ITERATOR, VALUE_INPUT_ITERATOR, PythonFunction<T, I, L>&){
         throw RuntimeError(" Cannot save/load gm with a pure python function: Pure Python Function cannot be serialized / deserialized");
      }
   };
}
...
namespace opengm{
   ...
   template<class T, class I, class L>
   struct FunctionRegistration<PythonFunction<T, I, L> > {
      enum ID {
