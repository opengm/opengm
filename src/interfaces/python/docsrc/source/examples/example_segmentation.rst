Interpixel Boundary Segmentation
--------------------------------
The following example performs an interpixel boundarie segmentation on a topological grid.
The code uses the function  "printSolution" and the class "TopologicalCoordinateToIndex".
The code for thes function/classes is given below the example code::

   import opengm,numpy,sys
...
TopologicalCoordinateToIndex is used to map the coorinates of the two dimensional topological
grid to a one dimensial variable index::
...
printSolution is used to visualize the result of the inference / optimiation::
