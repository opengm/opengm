      ...
      for r,d,f in os.walk("examples"):
         for files in f:
            if files.endswith(".py"):
               if(str(files).endswith('gui.py')==False):
                  pass
                  #execfile(filePath)
...
class TestUtilities:
   ...
   def test_is_build_in_simple_parameter(self):
      class MyClass(object):
         def __init__(self):
            pass
      assert( opengm._to_native_converter.is_build_in_simple_parameter(classType=MyClass)==False)
      assert( opengm._to_native_converter.is_build_in_simple_parameter(instanceType=MyClass())==False)   
      assert( opengm._to_native_converter.is_build_in_simple_parameter(classType=bool)==True)
      assert( opengm._to_native_converter.is_build_in_simple_parameter(instanceType=bool())==True)
      assert( opengm._to_native_converter.is_build_in_simple_parameter(instanceType=1)==True)
      assert( opengm._to_native_converter.is_build_in_simple_parameter(instanceType=1.0)==True)
      assert( opengm._to_native_converter.is_build_in_simple_parameter(instanceType='1.0')==True)
      simple_types=[int,long,float,bool,str]
      for st in simple_types:
         assert( opengm._to_native_converter.is_build_in_simple_parameter(classType=st)==True)
         assert( opengm._to_native_converter.is_build_in_simple_parameter(instanceType=st())==True)
   def test_is_tribool(self):
      assert( opengm._to_native_converter.is_tribool(classType=opengm.Tribool)==True)
      assert( opengm._to_native_converter.is_tribool(instanceType=opengm.Tribool(0))==True)
      assert( opengm._to_native_converter.is_tribool(classType=bool)==False)
      assert( opengm._to_native_converter.is_tribool(instanceType=True)==False)
...
class TestGm:     
   ...
   def test_factor_generators(self):
      ...
      functions=[numpy.ones(2,dtype=numpy.float32).reshape(2),
         ...
         numpy.ones(32,dtype=numpy.float32).reshape(2,2,2,2,2)]
      ...
      for order in xrange(1,6):
         ...
         gens=[gm.factors(minOrder=order),gm.factorIds(minOrder=order),gm.factorsAndIds(minOrder=order)]
         assert(len(gens)==3)
         for gen in gens:
            print "len assert"
      ...
      for order in xrange(2,6):
         orderSets=[set(),set(),set()]
         gens=[gm.factors(maxOrder=order),gm.factorIds(maxOrder=order),gm.factorsAndIds(maxOrder=order)]
         assert(len(gens)==3)
         for gen in gens:
            print "len assert"
      ...
      for order in xrange(1,6):
         orderSets=[set(),set(),set()]
         gens=[gm.factors(minOrder=order,maxOrder=4),gm.factorIds(minOrder=order,maxOrder=4),gm.factorsAndIds(minOrder=order,maxOrder=4)]
         assert(len(gens)==3)
         for gen in gens:
            print "len assert"
         ...
         gens=[gm.factors(minOrder=order,maxOrder=4),gm.factorIds(minOrder=order,maxOrder=4),gm.factorsAndIds(minOrder=order,maxOrder=4)]
         for  factor in gens[0]:
               assert factor.numberOfVariables >= order and factor.numberOfVariables <=4
               orderSets[0].add(factor.numberOfVariables)
         for  fId in gens[1]:
               assert gm[fId].numberOfVariables >=  order and gm[fId].numberOfVariables <=4
               orderSets[1].add(gm[fId].numberOfVariables)
         for  factor,fId in gens[2]:
               assert factor.numberOfVariables >=  order and factor.numberOfVariables <=4
               assert gm[fId].numberOfVariables >=  order and gm[fId].numberOfVariables <=4  
...
class Test_Inference():
   def __init__(self):
      self.gridGm={
         'adder'     :generate_grid(dimx=2,dimy=2,labels=2,beta1=0.1,beta2=0.2,operator='adder'),
         'multiplier':generate_grid(dimx=2,dimy=2,labels=2,beta1=0.1,beta2=0.2,operator='multiplier'),
      }
      
      self.gridGm3={
         'adder'     :generate_grid(dimx=3,dimy=2,labels=3,beta1=0.1,beta2=0.2,operator='adder'),
         'multiplier':generate_grid(dimx=3,dimy=2,labels=3,beta1=0.1,beta2=0.2,operator='multiplier'),
      }
      self.chainGm={
         'adder'     :generate_grid(dimx=4,dimy=1,labels=2,beta1=0.1,beta2=0.2,operator='adder'),
         'multiplier':generate_grid(dimx=4,dimy=1,labels=2,beta1=0.1,beta2=0.2,operator='multiplier')
      }
      self.chainGm3={
         'adder'     :generate_grid(dimx=4,dimy=1,labels=3,beta1=0.1,beta2=0.2,operator='adder'),
         'multiplier':generate_grid(dimx=4,dimy=1,labels=3,beta1=0.1,beta2=0.2,operator='multiplier')
      }
      
      self.all           = [ ('adder','minimizer'),('adder','maximizer') ,('multiplier','minimizer') ,('multiplier','maximizer')  ]
   ...
   def test_bruteforce(self):
      ...
      genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm ,self.gridGm3, self.chainGm3] ,semiRings=self.all)
   ...
   def test_icm(self):
      ...
      params      = [ None,opengm.InfParam(moveType='variable'),opengm.InfParam(moveType='factor'),opengm.InfParam() ]
      genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm ,self.gridGm3, self.chainGm3] ,semiRings=self.all)

   def test_lazyflipper(self):
      solverClass = opengm.inference.LazyFlipper
      params      = [ None,opengm.InfParam(maxSubgraphSize=2),opengm.InfParam() ]
      genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm ,self.gridGm3, self.chainGm3] ,semiRings=self.all)

   def test_loc(self):
      solverClass = opengm.inference.Loc
      params      = [ None,opengm.InfParam(phi=0.5),opengm.InfParam(phi=0.5,maxRadius=10,steps=100) ]
      genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm ,self.gridGm3, self.chainGm3] ,semiRings=self.all)
   ...
   def test_dualdecompostion_subgradient(self):
      ...
      try:
         genericSolverCheck(solverClass,params=params,gms=[self.gridGm, self.gridGm3, self.chainGm, self.chainGm3] ,semiRings=self.minSum)   
   ...
   def test_dualdecompostion_subgradient_dynamic_programming(self):
      solverClass = opengm.inference.DualDecompositionSubgradient
      params      = [ opengm.InfParam(subInference='dynamic-programming' ,subInfParam=opengm.InfParam() ),
         opengm.InfParam(subInference='dynamic-programming' ,decompositionId='tree',subInfParam=opengm.InfParam() )
      ]
      try :
         genericSolverCheck(solverClass,params=params,gms=[self.gridGm, self.gridGm3, self.chainGm, self.chainGm3] ,semiRings=self.minSum)  
      except RuntimeError as detail:  
   ...
   def test_dualdecompostion_subgradient_graph_cut(self):
      solverClass = opengm.inference.DualDecompositionSubgradient
      params      = [ opengm.InfParam(subInference='graph-cut' ,decompositionId='blocks',subInfParam=opengm.InfParam() )]
      try:
   ...
   def test_gibbs(self):
      ...
      genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm,self.gridGm3, self.chainGm3 ] ,semiRings=self.minSumMaxProd)  
   ...
   def test_trws_external(self):
      if opengm.configuration.withTrws:
         ...
         params      = [ None,opengm.InfParam(),opengm.InfParam(steps=100,energyType='view') ,opengm.InfParam(steps=1,energyType='tables')]
         genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm,self.gridGm3, self.chainGm3] ,semiRings=self.minSum) 

   def test_graphcut(self):
      solverClass = opengm.inference.GraphCut
      params      = [ None,opengm.InfParam(),opengm.InfParam(minStCut='boost-kolmogorov'),opengm.InfParam(minStCut='push-relabel')]
   ...
   def test_qpbo_external(self):
      if opengm.configuration.withQpbo:
         ...
         params      = [ None,opengm.InfParam(),opengm.InfParam(useProbeing=True),opengm.InfParam(strongPersistency=True) ,opengm.InfParam(useImproveing=True)  ]
         genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm] ,semiRings=self.minSum) 

   def test_alpha_beta_swap(self):
      solverClass = opengm.inference.AlphaBetaSwap
      params      = [ None,opengm.InfParam(steps=10),opengm.InfParam(minStCut='boost-kolmogorov',steps=10),opengm.InfParam(minStCut='push-relabel',steps=10)]
      if opengm.configuration.withMaxflow:
         params.append(opengm.InfParam(minStCut='kolmogorov',steps=10) )
      genericSolverCheck(solverClass,params=params,gms=[ self.gridGm3, self.chainGm3] ,semiRings=self.minSum)
   ...
   def test_lpcplex(self):
      if opengm.configuration.withCplex:
         ...
         params      = [ None,opengm.InfParam(),opengm.InfParam(integerConstraint=True),opengm.InfParam(integerConstraint=False)]
         genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm,self.gridGm3, self.chainGm3] ,semiRings=self.minSum) 
   ...
   def test_libdai_bp(self):
      if opengm.configuration.withLibdai:
         ...
         params      = [ None,opengm.InfParam(),opengm.InfParam(updateRule='parall'),opengm.InfParam(updateRule='seqrnd')]
         genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm,self.gridGm3, self.chainGm3] ,semiRings=self.minSum) 
   ...
   def test_libdai_trw_bp(self):
      if opengm.configuration.withLibdai:
         ...
         params      = [ None,opengm.InfParam(),opengm.InfParam(updateRule='parall'),opengm.InfParam(updateRule='seqrnd',ntrees=2)]
         genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm,self.gridGm3, self.chainGm3] ,semiRings=self.minSum) 

   def test_libdai_gibbs(self):
      if opengm.configuration.withLibdai:
         solverClass = opengm.inference.GibbsLibDai
         params      = [ None,opengm.InfParam(),opengm.InfParam(steps=100)]
         genericSolverCheck(solverClass,params=params,gms=[ self.gridGm, self.chainGm,self.gridGm3, self.chainGm3] ,semiRings=self.minSum) 
