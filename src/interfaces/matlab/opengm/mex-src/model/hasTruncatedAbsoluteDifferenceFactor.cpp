...
/**
 * @brief This file implements an interface to retrieve the information if a given model has at least one truncated absolute difference factor.
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  // check if model has TL1 factor
  bool hasTL1 = false;
  for(GmType::IndexType i = 0; i < numFactors; i++) {
     if(gm[i].isTruncatedAbsoluteDifference()) {
        hasTL1 = true;
        break;
     }
  }
  ...
  // return result
  plhs[0] = mxCreateLogicalScalar(hasTL1);
}
