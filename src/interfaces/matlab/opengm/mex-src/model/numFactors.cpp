...
/**
 * @brief This file implements an interface to retrieve the number of factors from a given model.
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if(size > 8) {
     ...
     reinterpret_cast<uint64_T*>(mxGetData(plhs[0]))[0] = static_cast<uint64_T>(numFactors);
  } else if(size > 2) {
     plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT32_CLASS, mxREAL);
     reinterpret_cast<uint32_T*>(mxGetData(plhs[0]))[0] = static_cast<uint32_T>(numFactors);
  } else if(size > 1) {
     plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT16_CLASS, mxREAL);
     reinterpret_cast<uint16_T*>(mxGetData(plhs[0]))[0] = static_cast<uint16_T>(numFactors);
  } else {
     plhs[0] = mxCreateNumericMatrix(1, 1, mxUINT8_CLASS, mxREAL);
     reinterpret_cast<uint8_T*>(mxGetData(plhs[0]))[0] = static_cast<uint8_T>(numFactors);
  }
}
